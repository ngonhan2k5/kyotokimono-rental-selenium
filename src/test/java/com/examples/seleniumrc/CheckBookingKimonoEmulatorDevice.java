...
public class CheckBookingKimonoEmulatorDevice {
	...
	public void checkBookingKimonoStandard() throws InterruptedException {
		...
		price_dress = Integer.parseInt(getAttributeElement(".//*[@id='person_amount']", "data-value"));
		...
		//Thread.sleep(500);
		 ...
		 checkPriceOptionTable(num_person));
	...
	public Boolean chooseRanDomDateOneTable() throws InterruptedException {
		...
		for (int j = 1; j <= 7; j++) {
			...
			for (int i = 1; i <= row; i++) {
				...
				xpathDateElement = ".//*[@id='choose-date']/div[2]/div/table/tbody/tr[" + indr_date + "]/td["
						+ indc_date + "]/div/div";
	...
	public Boolean checkPricePhotoTable() throws InterruptedException {
		...
		System.out.println(sum+"...."+sum_photo);
		if (sum == sum_photo) {
			return true;
		} else {
