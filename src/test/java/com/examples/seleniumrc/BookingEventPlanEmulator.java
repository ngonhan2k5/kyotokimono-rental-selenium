...
public class BookingEventPlanEmulator {
	...
	public void chooesOneDress() throws InterruptedException {
		...
		nameShop = "osaka";
		// nameShop = "tokyo";
		chooseShopAndDress(nameShop, 3);// a is a first dress in list
		Thread.sleep(2000);
		// choose date
		chooseRanDomDate();
	...
	public void checkPriceOptionTable() throws InterruptedException {
		...
		int optionTable1 = getRowTableDate("#product-" + iddress + " .group-option.group-option-2 li");
		int optionTable2 = getRowTableDate("#product-" + iddress + " .group-option.group-option-3 li");
		int optionTable3 = getRowTableDate("#product-" + iddress + " .group-option.group-option-4 li");
		int optionTable4 = getRowTableDate("#product-" + iddress + " .group-option.group-option-5 li");
		
		
		
		sum += getPriceOptionOneTable("#product-" + iddress + " .group-option.group-option-2 li:nth-child(", 3, optionTable1)
				+ getPriceOptionOneTable("#product-" + iddress + " .group-option.group-option-3 li:nth-child(", 2, optionTable2)
				+ getPriceOptionOneTable("#product-" + iddress + " .group-option.group-option-4 li:nth-child(", 2, optionTable3)
				+ getPriceOptionOneTable("#product-" + iddress + " .group-option.group-option-5 li:nth-child(", 2, optionTable4);
	...
	public void chooseOption() throws InterruptedException {
		...
		Thread.sleep(1000);
		scrollAndClickCSS(".fa-icon-collapsed.icon-click.icon-fa-collapsed-down",1000);
		Thread.sleep(1000);
		 clickButtonCss("[data-id='11'] label ");
	...
	public void chooseDress(int amountdress, int indexdress) throws NumberFormatException, InterruptedException {
		...
		dressName = findCss(".list.dp-flex li:nth-child(" + Integer.toString(indexdress) + ") .product-name:nth-child(2)").getText()
				.replace("\n", "");
...
//				findCss(" li:nth-child(" + indexdress + ") .price a").getText().replace(",", "").replace("円", ""));
	...
	public void chooseRanDomDate() throws InterruptedException {
		...
		while (!chooseRanDomDateOneTable()) {
			scrollAndClickXpath(".//*[@id='page-next']",500);
			Thread.sleep(3000);
	...
	public void checkTitleDateMessageTable() throws InterruptedException {
		...
		getTiltleMessage += findXpath(
				".//*[@id='choose-date']/div[2]/div/table/thead/tr/td[" + Integer.toString(indcFirstSelectedDate) + "]")
						.getText().replaceAll("[0-9]", "").replace("/", "").replace(".", "").trim()
				+ ")";
	...
	public void checkMessageDateTable() throws InterruptedException {
		...
		for (int i = 1; i <= rowsMessageProductPage; i++) {
			...
			messageDateProductpageList.add(findCss("#allocate-info-hour-list li:nth-child(" + Integer.toString(i) + ")")
					.getText().replace("\n", ""));
		}
	...
	public void inputCustomerInfomation() throws InterruptedException {
		...
		scrollAndClickCSS("#select2-HighendBook_birthday_day-container", 0);
		scrollAndClickCSS("#HighendBook_birthday_day option:nth-child(5)", 0);
